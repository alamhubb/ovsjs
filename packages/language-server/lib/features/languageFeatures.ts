...
export function register(
  ...
  server.onInitialize(serverCapabilities => {
    ...
    LogUtil.log('=== Semantic Tokens Diagnostic ===');
    LogUtil.log('Total plugins:', languageServicePlugins.length);
    LogUtil.log('Plugins with semantic tokens:',
      languageServicePlugins.filter(p => p.capabilities.semanticTokensProvider).map(p => p.name)
    );
    if (languageServicePlugins.some(({capabilities}) => capabilities.semanticTokensProvider)) {
      LogUtil.log('✅ Registering semantic tokens handler');
      ...
      server.connection.languages.semanticTokens.on(async (params, token, _, resultProgress) => {
        const uri = URI.parse(params.textDocument.uri);
        LogUtil.log("server.connection.languages.semanticTokens.on(")
        return await worker(uri, token, async languageService => {
          ...
          LogUtil.log("return tokens  .connection.languages.semanticTokens.on ")
          LogUtil.log(result)
          return result
        }) ?? {data: []};
      });
      ...
      LogUtil.log('❌ NO plugin provides semanticTokensProvider - handler NOT registered');
    }

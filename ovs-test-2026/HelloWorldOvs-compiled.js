...
export default defineOvsComponent(props => {
  ...
  return defineOvsComponent(() => {
    const children = [];
    const temp$$attrs$$8jdo3ry8 = {};
    children.push($OvsHtmlTag.h1({},[
      "Level 1: ",
    ...
    children.push(defineReactiveExpression(() => [1,2,3].map(item => $OvsHtmlTag.div({},[
    ...
    console.log("这是副作用代码，不会被渲染");
    let sideEffectVar = count1.value * 2;
    return $OvsHtmlTag.div({class:"level-1",style:"border: 2px solid red; padding: 20px; margin: 10px;"},children);
  })({},[]);
});

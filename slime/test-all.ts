...
// Slime测试 - ES6 Parser完整测试套件（40个用例）
// 用途：运行全部测试，统计通过率，生成报告
// 日常修复：使用 test-runner.ts（单例测试）
...
async function runTests() {
  ...
  console.log('║' + ' 预计耗时: 约85秒'.padEnd(78, ' ') + '║')
  console.log('╚' + '═'.repeat(78) + '╝\n')
  ...
  console.log('\n' + '═'.repeat(80))
  console.log(`📊 测试总结: ${passCount}/${testCases.length} 通过 (${(passCount/testCases.length*100).toFixed(1)}%)`)

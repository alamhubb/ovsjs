...
console.log('Test 1: function f() { return 1 }')
console.log('='.repeat(60))
...
try {
  ...
  console.log(`Parser curIndex: ${(parser1 as any).curIndex} / ${tokens1.length}`)
} catch (e) {
  console.error('Error:', e)
}

console.log('\n' + '='.repeat(60))
console.log('Test 2: function f() { x = 1 }')
console.log('='.repeat(60))
...
try {
  ...
  console.log(`Parser curIndex: ${(parser2 as any).curIndex} / ${tokens2.length}`)
  
  if ((parser2 as any).curIndex < tokens2.length) {
    console.log(`Next uncons

umed token: ${tokens2[(parser2 as any).curIndex].tokenName}`)
  }
} catch (e) {
  console.error('Error:', e)
}

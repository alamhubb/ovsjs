/**
 * 测试禁用 memoization 后是否正常
 */
import Es2020Parser from "./packages/slime-parser/src/language/es2020/Es2020Parser";
import {es2020Tokens} from "./packages/slime-parser/src/language/es2020/Es2020Tokens";
import SubhutiLexer from '../subhuti/src/parser/SubhutiLexer.ts'

function collectTokenValues(node: any): string[] {
    const values: string[] = []
    
    if (node.value !== undefined && (!node.children || node.children.length === 0)) {
        values.push(node.value)
    }
    
    if (node.children) {
        for (const child of node.children) {
            values.push(...collectTokenValues(child))
        }
    }
    
    return values
}

function testWithMemo(code: string, enableMemo: boolean) {
    const memoStatus = enableMemo ? '启用' : '禁用'
    console.log(`\n【${memoStatus} Memoization】`)
    console.log('代码:', code)
    console.log('-'.repeat(40))
    
    try {
        const lexer = new SubhutiLexer(es2020Tokens)
        const tokens = lexer.lexer(code)
        const parser = new Es2020Parser(tokens)
        
        // 设置 memoization
        parser.enableMemoization = enableMemo
        
        const cst = parser.Program()
        const cstTokens = collectTokenValues(cst)
        
        console.log(`✅ 成功: ${cstTokens.length}个tokens`)
        console.log(`Tokens:`, cstTokens.join(', '))
        
        const inputTokens = tokens
            .filter((t: any) => {
                const tokenName = t.tokenType?.name || ''
                return tokenName !== 'SingleLineComment' &&
                    tokenName !== 'MultiLineComment' &&
                    tokenName !== 'Spacing' &&
                    tokenName !== 'LineBreak'
            })
            .map((t: any) => t.tokenValue)
        
        const missingTokens = inputTokens.filter((v: string) => !cstTokens.includes(v))
        
        if (missingTokens.length > 0) {
            console.log(`❌ 丢失 ${missingTokens.length} 个tokens:`, missingTokens)
        } else {
            console.log(`✅ Token完整`)
        }
        
    } catch (error: any) {
        console.log(`❌ 失败:`, error.message.slice(0, 100))
    }
}

const testCases = [
    `Math.max(1, 2)`,
    `foo()`,
    `Math.max(1, 2) + Math.min(5, 3)`
]

console.log('='.repeat(60))
console.log('测试：Memoization 对解析的影响')
console.log('='.repeat(60))

for (const code of testCases) {
    console.log(`\n${'='.repeat(60)}`)
    console.log(`测试代码: ${code}`)
    testWithMemo(code, true)   // 启用 memoization
    testWithMemo(code, false)  // 禁用 memoization
}

console.log(`\n${'='.repeat(60)}`)


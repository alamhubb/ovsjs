...
import { ovsInheritedSyntaxSource } from '../../ovs/ovs-compiler/tests/generated-parser-chain-fixture.ts'
...
const compilerTsdownConfigPath = path.join(workspaceRoot, 'ovs', 'ovs-compiler', 'tsdown.config.ts')
...
async function main() {
  ...
  requireIncludes(parserSource, 'Alternative,', 'OvsParser.ts')
  ...
  requireIncludes(parserSource, 'TSInterfaceDeclaration', 'OvsParser.ts')
  requireIncludes(parserSource, 'TSTypeAliasDeclaration', 'OvsParser.ts')
  requireIncludes(parserSource, 'getTokenConsumer().Export()', 'OvsParser.ts')
